...
if ( ! class_exists( 'SLFW_Module_Dependence' ) ) {

    class SLFW_Module_Dependence {
        ...
        public function admin_notices() {
            foreach ( $this->notices as $notice ) {
                $notice = $notice;
                include_once SLFW_PLUGIN_PATH . '/modules/dependence/includes/views/html-notice.php';
            }
        }
        ...
        private function create_install_plugin_notice( $plugin ) {
            ...
            return sprintf(
                ...
                __( '<strong>Shipping Loggi for WooCommerce</strong> depends of %1$s to work. Click to %2$s.', 'shipping-loggi-for-woocommerce' ),
                $plugin->name,
                '<a href="' . esc_url( $url ) . '">' . __( 'install the plugin', 'shipping-loggi-for-woocommerce' ) . '</a>'
            );
        }
        ...
        private function create_activate_plugin_notice( $plugin ) {
            ...
            return sprintf(
                ...
                __( '<strong>Shipping Loggi for WooCommerce</strong> depends of %1$s to work. Click to %2$s.', 'shipping-loggi-for-woocommerce' ),
                $plugin->name,
                '<a href="' . esc_url( $url ) . '">' . __( 'activate the plugin', 'shipping-loggi-for-woocommerce' ) . '</a>'
            );
        }
